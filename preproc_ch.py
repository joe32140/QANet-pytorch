...
def process_file(filename, data_type, word_counter, char_counter):
    ...
    with open(filename, "r") as fh:
        ...
        for article in tqdm(source):
            seg_context = article["conversations"].replace(
                "''", '" ').replace("``", '" ')
            context_tokens = word_tokenize(seg_context)
            ...
            #Question and Answer
            total += 1
            seg_ques = article["question"].replace(
                "''", '" ').replace("``", '" ')
            ques_tokens = word_tokenize(seg_ques)
            ques_chars = [list(token) for token in ques_tokens]
            ques = ''.join(ques_tokens)
            ...
            answer_text = article["answers"][0].replace(' ','')
            answer_start = article['answer_start']
            ...
            eval_examples[str(total)] = {
                "context": context, "spans": spans, "answers": answer_texts, "uuid": article["q_id"]}
...
def get_embedding(counter, data_type, limit=-1, emb_file=None, vec_size=None):
    ...
    if emb_file is not None:
        ...
        vocab = json.load(open('../data/300d.trad_chinese.word2id.json'))
        vectors = np.load(emb_file)
        for word in tqdm(vocab.keys()):
            if word in counter and counter[word] > limit:
                #print(vectors[vocab[word]])
                embedding_dict[word] = vectors[vocab[word]]
...
def preproc(config):
    ...
    save_emb(config.char_emb_file, char_emb_mat, message="char embedding")
